...
let currentIndex = 1; // Начинаем со второго изображения

function updateCarouselPosition() {
    const carouselInner = document.querySelector('.image-carousel-inner');
    const offset = -currentIndex * 100 / 2; // Смещение на 50% за каждый слайд
    carouselInner.style.transform = `translateX(${offset}%)`;
}

function imageNextSlide() {
    const slides = document.querySelectorAll('.image-carousel-item');
    if (currentIndex < slides.length - 2) { // Учитываем отображение двух половин
        currentIndex++;
        updateCarouselPosition();
    }
}

function imagePrevSlide() {
    if (currentIndex > 0) {
        currentIndex--;
        updateCarouselPosition();
    }
